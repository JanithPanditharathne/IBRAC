...
# Package structure
#
# event-accumulator-service-[VERSION].zip/tar.gz
#     |- # event-accumulator-service-[VERSION]
#           |- eas.sh
#           |- bin
#           |   |- eas.jar
#           |- conf
...
cp "${TARGET_FOLDER}/${ARTIFACT_ID}-${ARTIFACT_VERSION}.jar" "$PACKAGE_DIR/bin/eas.jar"
cp scripts/eas.sh "$PACKAGE_DIR/."
cp src/main/resources/application.properties "$PACKAGE_DIR/conf/."
cp src/main/resources/logback.xml "$PACKAGE_DIR/conf/."
cp "src/main/environmentConfigs/override.properties" "$PACKAGE_DIR/conf/override.properties"
